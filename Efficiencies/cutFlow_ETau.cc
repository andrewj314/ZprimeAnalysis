...
const double cand1_acc[2]       = {23, 2.1};
...
const int numTotCut       = 17; //dr+cand1_numTotCut+cand2_numTotCut+pair_numTotCut 
...
//  b_Muon_besttrack_dxy->GetEntry(tentry);
//  b_Muon_besttrack_dz->GetEntry(tentry);
  ...
  b_Tau_againstElectronMVATightMVA5->GetEntry(tentry);
  ...
  if(patElectron_pt->size() > 0 && Tau_pt->size() > 0){
    if(noPUcorr) wgt_pu = 1.;
    cut_eff[0] += wgt_pu;
    if(patElectron_pt->at(0) >= cand1_acc[0]){
      cut_eff[1] += wgt_pu;
      cout << "1\n";
      if(patElectron_eta->at(0) < cand1_acc[1]){
        cut_eff[2] += wgt_pu;
        cout << "2\n";
        if(Tau_pt->at(0) >= cand2_acc[0]){
          cut_eff[3] += wgt_pu;
	  cout << "3\n";
          if(Tau_eta->at(0) < cand2_acc[1]){
            cut_eff[4] += wgt_pu;
	    cout << "4\n";
            if(patElectron_gsfTrack_dxy_pv->at(0) < 0.045 && patElectron_gsfTrack_dz_pv->at(0) < 0.2){
              cut_eff[5] += wgt_pu;
	      cout << "5\n";
		...
		if(patElectron_expectedMissingInnerHits->at(0) <= 1){
                  cut_eff[7] += wgt_pu;
		  if(patElectron_passConversionVeto->at(0)){
		    cut_eff[8] += wgt_pu;
		      ...
		      cout << "7\n";
                      if(Tau_leadChargedCandDz_pv->at(0) < 0.2){
                        cut_eff[10] += wgt_pu;
		        cout << "8\n";
		        double DeltaR = deltaR(patElectron_eta->at(0), patElectron_phi->at(0), Tau_eta->at(0), Tau_phi->at(0));
		        if(DeltaR > 0.5){
		          cut_eff[11] += wgt_pu;
		          cout << "9\n";
		          double Electron_Iso;
		          ...
		          if(Electron_Iso < 0.1){
		            cut_eff[12] += wgt_pu;
			    cout << "10\n";
		            if(Tau_againstElectronMVATightMVA5->at(0) > 0.5){
			      cut_eff[13] += wgt_pu;
			      cout << "11\n";
			      if(Tau_againstMuonLoose3->at(0) > 0.5){
				    ...
				    double patElectron_Iso_1 = (patElectron_isoChargedHadrons->at(1) + max(patElectron_isoNeutralHadrons->at(1) + patElectron_isoPhotons->at(1) - 0.5*patElectron_isoPU->at(1), 0.0))/patElectron_pt->at(1);
			            if(!(patElectron_pt->at(1) > 15 && fabs(patElectron_eta->at(1)) < 2.5 
				    && patElectron_gsfTrack_dxy_pv->at(1) < 0.045 && patElectron_gsfTrack_dz_pv->at(1) < 0.2 && patElectron_Iso_1 < 0.3)){
			              cut_eff[16] += wgt_pu;
				      cout << "14b\n"; 
				    }
			          }  
...
/////
void initialize_strings(vector<string> &varname, vector<string> &varcut){
  varname[0] = "e-tau pair exists:\t";
  varname[1] = "Electron Pt:\t\t"; varname[2] = "Electron eta:\t\t";
  varname[3] = "Tau Pt:\t\t\t"; varname[4] = "Tau eta:\t\t";
  varname[5] = "Electron track dxy & dz:"; varname[6] = "Electron ID:\t\t";
  varname[7] = "Electron missing hits:\t"; varname[8] = "Electron Conversion veto:";
  varname[9] = "Tau DMF new DMs:\t"; varname[10] = "Tau dz:\t\t\t";
  varname[11] = "ETau DeltaR:\t\t"; varname[12] = "Electon Iso:\t\t";
  varname[13] = "Tau anti-e MVA5:\t"; varname[14] = "Tau anti-mu Loose:\t";
  varname[15] = "Tau Medium Iso DB 3hits:"; varname[16] = "Di-electron veto:\t";
