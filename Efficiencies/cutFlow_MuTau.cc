...
const int numTotCut       = 15; //dr+cand1_numTotCut+cand2_numTotCut+pair_numTotCut 
		...
		cout << "6\n";
		  ...
		  cout << "7\n";
                  if(Tau_leadChargedCandDz_pv->at(0) < 0.2){
                    cut_eff[8] += wgt_pu;
		    cout << "8\n";
		    double DeltaR = deltaR(Muon_eta->at(0), Muon_phi->at(0), Tau_eta->at(0), Tau_phi->at(0));
		    if(DeltaR > 0.5){
		      cut_eff[9] += wgt_pu;
		      cout << "9\n";
		      double Muon_Iso;
			...
			cout << "10\n";
			  ...
			  if(Tau_againstMuonTight3->at(0) > 0.5){
				...
				double Muon_Iso_1 = (Muon_isoCharged->at(1) + max(Muon_isoNeutralHadron->at(1) + Muon_isoPhoton->at(1) - 0.5*Muon_isoPU->at(1), 0.0))/Muon_pt->at(1);
			        if(!(Muon_pt->at(1) > 15 && fabs(Muon_eta->at(1)) < 2.4 && Muon_isGlobal->at(1) && Muon_isTrackerMuon->at(1) 
				&& Muon_pf->at(1) && Muon_dxy_pv->at(1) < 0.045 && Muon_dz_pv->at(1) < 0.2 && Muon_Iso_1 < 0.3)){
			          cut_eff[14] += wgt_pu;
				  cout << "14b\n"; 
				}
			      }  
...
/////
void initialize_strings(vector<string> &varname, vector<string> &varcut){
  varname[0] = "mu-tau pair exists:\t";
  varname[1] = "Muon Pt:\t\t"; varname[2] = "Muon eta:\t\t";
  varname[3] = "Tau Pt:\t\t\t"; varname[4] = "Tau eta:\t\t";
  varname[5] = "Muon track dxy & dz:\t"; varname[6] = "Muon ID:\t\t";
  varname[7] = "Tau DMF new DMs:\t"; varname[8] = "Tau dz:\t\t\t";
  varname[9] = "MuTau DeltaR:\t\t"; varname[10] = "Muon Iso:\t\t";
  varname[11] = "Tau anti-e MVA5:\t"; varname[12] = "Tau anti-mu tight:\t";
  varname[13] = "Tau Medium Iso DB 3hits:"; varname[14] = "Di-muon veto:\t\t";
