...
const bool decayMode_plots = false;
//const int const_bin = 12; const double const_from = 0; const double const_to = 12;
//const string titleXaxis = "Tau Decay Mode"; const int lmcol = 2;
//const string part = "decayMode_";
...
//   Main function
/////
void TauTau_Plots_EffpT_Effeta_Effpu_PogStudy(){
 setTDRStyle();
 //Call tree 
 TFile* f = Call_TFile();
 TTree* tree; 
...
void cutflow_eff_obj(TTree* tree, int nentries){ 
 ...
 TH1I* decayModesBeforeMatching = new TH1I("decayModesBeforeMatching", "Decay Modes Before Matching", 20, 0, 20);
 TH1I* decayModesAfterMatching = new TH1I("decayModesAfterMatching", "Decay Modes After Matching", 20, 0, 20);

 TH1F* RecoTauPtHisto = new TH1F("RecoTauPtHisto", "Reco Tau p_{T}", 100, 0, 2000);
 TH1F* GenTauPtHisto = new TH1F("GenTauPtHisto", "Gen Tau p_{T}", 100, 0, 2000);
 ...
 //Jets
 vector<double> *Jet_pt, *Jet_eta, *Jet_phi, *Jet_energy;
 Jet_pt = 0;
 Jet_eta = 0;
 Jet_phi = 0;
 Jet_energy = 0;
 ...
 tree->SetBranchAddress("Jet_energy",&Jet_energy,&b_Jet_energy);
 ...
 //Jets
  ...
  if(genHadronicTauPairExists(tree, i).first && Tau_pt->size() >= 2){
    for(int j = 0; j < Tau_pt->size(); j++){
      ...
      if(matchTauToGen(recoCand, tree, i).first){
        matchedTaus++;
        tauResolution->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
        ...
        RecoTauPtHisto->Fill(recoCand.Pt());
  	GenTauPtHisto->Fill(matchTauToGen(recoCand,tree,i).second.Pt());
        ...
        if(matchTauToGen(recoCand,tree,i).second.Pt() < 100) tauResolution0to100->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
        if(matchTauToGen(recoCand,tree,i).second.Pt()>100 && matchTauToGen(recoCand,tree,i).second.Pt()<200){
	 tauResolution100to200->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
	}
        if(matchTauToGen(recoCand,tree,i).second.Pt()>200 && matchTauToGen(recoCand,tree,i).second.Pt()<350){
	 tauResolution200to350->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
	}
        if(matchTauToGen(recoCand,tree,i).second.Pt()>350 && matchTauToGen(recoCand,tree,i).second.Pt()<500){
	 tauResolution350to500->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
	}
        if(matchTauToGen(recoCand,tree,i).second.Pt()>500 && matchTauToGen(recoCand,tree,i).second.Pt()<750){
	 tauResolution500to750->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
	}
        if(matchTauToGen(recoCand,tree,i).second.Pt()>750 && matchTauToGen(recoCand,tree,i).second.Pt()<1000){
	 tauResolution750to1000->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
	}
        if(matchTauToGen(recoCand,tree,i).second.Pt()>1000){
	 tauResolution1000toInf->Fill((fabs(recoCand.Pt()) - fabs(matchTauToGen(recoCand, tree, i).second.Pt()))/fabs(matchTauToGen(recoCand,tree,i).second.Pt()));
	}
      }
    }
  }
    ...
    if(fabs(Tau_eta->at(0)) < cand2_acc[1]){
     ...
     if(Tau_decayModeFindingNewDMs->at(0) > 0.5){
      ...
      if(Tau_leadChargedCandDz_pv->at(0) < 0.2){
       cut_eff[4][i] = 1;
       TauCumId_d->Fill(Tau_pt->at(0));
       if(Tau_againstElectronMVALooseMVA5->at(0) > 0.5 && Tau_againstMuonLoose3->at(0) > 0.5 && Tau_byLooseCombinedIsolationDeltaBetaCorr3Hits->at(0) > 0.5){
	TauCumId_n->Fill(Tau_pt->at(0));
       }
       againstElectronLoose_d->Fill(Tau_pt->at(0));
       againstElectronMedium_d->Fill(Tau_pt->at(0));
       if(Tau_againstElectronMVAMediumMVA5->at(0) > 0.5) againstElectronMedium_n->Fill(Tau_pt->at(0));
       if(Tau_againstElectronMVALooseMVA5->at(0) > 0.5){
        againstElectronLoose_n->Fill(Tau_pt->at(0));
        cut_eff[5][i] = 1;
        againstMuonLoose3_d->Fill(Tau_pt->at(0));
        againstMuonTight3_d->Fill(Tau_pt->at(0));
        if(Tau_againstMuonLoose3->at(0)) againstMuonLoose3_n->Fill(Tau_pt->at(0));
        if(Tau_againstMuonTight3->at(0) > 0.5){
         againstMuonTight3_n->Fill(Tau_pt->at(0));
         cut_eff[6][i] = 1;
         rawCombinedIsoDB3Hits->Fill(Tau_byCombinedIsolationDeltaBetaCorrRaw3Hits->at(0));
         loosePileupWeightedIso3Hits_d->Fill(Tau_pt->at(0));
         mediumPileupWeightedIso3Hits_d->Fill(Tau_pt->at(0));
         tightPileupWeightedIso3Hits_d->Fill(Tau_pt->at(0));
	 if(Tau_byLoosePileupWeightedIsolation3Hits->at(0) > 0.5) loosePileupWeightedIso3Hits_n->Fill(Tau_pt->at(0));
	 if(Tau_byMediumPileupWeightedIsolation3Hits->at(0) > 0.5) mediumPileupWeightedIso3Hits_n->Fill(Tau_pt->at(0));
	 if(Tau_byTightPileupWeightedIsolation3Hits->at(0) > 0.5) tightPileupWeightedIso3Hits_n->Fill(Tau_pt->at(0));
         looseCombinedIsoDB3Hits_d->Fill(Tau_pt->at(0));
         mediumCombinedIsoDB3Hits_d->Fill(Tau_pt->at(0));
         tightCombinedIsoDB3Hits_d->Fill(Tau_pt->at(0));
         if(Tau_byLooseCombinedIsolationDeltaBetaCorr3Hits->at(0) > 0.5) looseCombinedIsoDB3Hits_n->Fill(Tau_pt->at(0)); 
         if(Tau_byTightCombinedIsolationDeltaBetaCorr3Hits->at(0) > 0.5) tightCombinedIsoDB3Hits_n->Fill(Tau_pt->at(0));
         if(Tau_byMediumCombinedIsolationDeltaBetaCorr3Hits->at(0) > 0.5){
          mediumCombinedIsoDB3Hits_n->Fill(Tau_pt->at(0));
          cut_eff[7][i] = 1;
 ...
 Double_t scalefactor = 1/tauMultiplicity->Integral();
 //tauMultiplicity->Scale(scalefactor); 
...
*/
 ...
 c1->cd();
 tauResolution->Draw();
 c1->SaveAs("TauResolution_FullScale_12_17_15.pdf");
 TCanvas *c2 = new TCanvas();
 c2->cd();
 tauResolution0to100->Draw();
 c2->SaveAs("TauResolution_0to100_12_17_15.pdf");

 TCanvas *c3 = new TCanvas();
 c3->cd();
 tauResolution100to200->Draw();
 c3->SaveAs("TauResolution_100to200_12_17_15.pdf");

 TCanvas *c4 = new TCanvas();
 c4->cd();
 tauResolution200to350->Draw();
 c4->SaveAs("TauResolution_200to350_12_17_15.pdf");

 TCanvas *c5 = new TCanvas();
 c5->cd();
 tauResolution350to500->Draw();
 c5->SaveAs("TauResolution_350to500_12_17_15.pdf");

 TCanvas *c6 = new TCanvas();
 c6->cd();
 tauResolution500to750->Draw();
 c6->SaveAs("TauResolution_500to750_12_17_15.pdf");

 TCanvas *c7 = new TCanvas();
 c7->cd();
 tauResolution750to1000->Draw();
 c7->SaveAs("TauResolution_750to1000_12_17_15.pdf");
 
 TCanvas *c8 = new TCanvas();
 c8->cd();
 tauResolution1000toInf->Draw();
 c8->SaveAs("TauResolution_1000toInf_12_17_15.pdf");
...
/*
 TCanvas *c0 = new TCanvas();
 c0->cd();
 TGraphAsymmErrors* theIDRatio = new TGraphAsymmErrors(TauCumId_n,TauCumId_d);
 theIDRatio->SetMaximum(12);
 theIDRatio->Divide(TauCumId_n, TauCumId_d);
 theIDRatio->SetTitle("Tau Cumulative ID");
 theIDRatio->SetLineColor(lmcol);
 theIDRatio->SetMarkerColor(lmcol);
 string titleYaxis;
  titleYaxis = "#epsilon(#tau ID)";
 theIDRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theIDRatio->GetYaxis()->SetTitle(titleYaxis.c_str());
 theIDRatio->GetYaxis()->SetRangeUser(0,1.25);
 TLegend *legend = new TLegend(0.65, 0.525, 0.9, 0.825);
 legend->AddEntry(theIDRatio, "Cumulative ID", "lep");
 
 theIDRatio->Draw("PAZ9");
 legend->Draw("same");
*/
/*
  TCanvas *c1 = new TCanvas();
  c1->cd();
  TGraphAsymmErrors* theDMFRatio = new TGraphAsymmErrors(Tau_DMFNMinus1_n,Tau_DMFNMinus1_d);
  theDMFRatio->BayesDivide(Tau_DMFNMinus1_n, Tau_DMFNMinus1_d);
  theDMFRatio->SetTitle("Tau DMF NMinus1");
  theDMFRatio->SetLineColor(lmcol);
  theDMFRatio->SetMarkerColor(lmcol);
  string titleYaxis1;
  titleYaxis1 = "#epsilon(DMF)";
  theDMFRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
  theDMFRatio->GetYaxis()->SetTitle(titleYaxis1.c_str());
  theDMFRatio->GetYaxis()->SetRangeUser(0,1.25);
  TGraphAsymmErrors* theDMFNewDMsRatio = new TGraphAsymmErrors(Tau_DMFNewDMsNMinus1_n,Tau_DMFNewDMsNMinus1_d);
  theDMFNewDMsRatio->BayesDivide(Tau_DMFNewDMsNMinus1_n, Tau_DMFNewDMsNMinus1_d);
  theDMFNewDMsRatio->SetTitle("Tau DMF NMinus1");
  theDMFNewDMsRatio->SetLineColor(lmcol+2);
  theDMFNewDMsRatio->SetMarkerColor(lmcol+2);
  string titleYaxis2;
   titleYaxis2 = "#epsilon(DMF)";
  theDMFNewDMsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
  theDMFNewDMsRatio->GetYaxis()->SetTitle(titleYaxis2.c_str());
  theDMFNewDMsRatio->GetYaxis()->SetRangeUser(0,1.25);

  TLegend *legend1 = new TLegend(0.65, 0.525, 0.9, 0.825);
  legend1->AddEntry(theDMFRatio, "DMF", "lep");
  legend1->AddEntry(theDMFNewDMsRatio, "DMF New DMs", "lep");
  theDMFRatio->Draw("PAZ9");
  theDMFNewDMsRatio->Draw("P same");
  legend1->Draw("same");
*/
...
/*
 ...
 theDMFRatio->GetYaxis()->SetTitle(titleYaxis1.c_str());
 theDMFRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theDMFNewDMsRatio = new TGraphAsymmErrors(DMFNewDMs_n,DMFNewDMs_d);
 theDMFNewDMsRatio->BayesDivide(DMFNewDMs_n, DMFNewDMs_d);
 theDMFNewDMsRatio->SetTitle("Tau DMF");
 theDMFNewDMsRatio->SetLineColor(lmcol+2);
 theDMFNewDMsRatio->SetMarkerColor(lmcol+2);
 string titleYaxis2;
  titleYaxis2 = "#epsilon(DMF)";
...
/*
 TCanvas *c2 = new TCanvas();
 c2->cd();
 TGraphAsymmErrors* theAgainstElectronLooseRatio = new TGraphAsymmErrors(againstElectronLoose_n,againstElectronLoose_d);
 theAgainstElectronLooseRatio->BayesDivide(againstElectronLoose_n, againstElectronLoose_d);
 theAgainstElectronLooseRatio->SetTitle("Tau AgainstElectronMVA5");
 theAgainstElectronLooseRatio->SetLineColor(lmcol);
 theAgainstElectronLooseRatio->SetMarkerColor(lmcol);
 string titleYaxis3;
  titleYaxis3 = "#epsilon(AgainstElectronMVA5)";
 theAgainstElectronLooseRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theAgainstElectronLooseRatio->GetYaxis()->SetTitle(titleYaxis3.c_str());
 theAgainstElectronLooseRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theAgainstElectronMediumRatio = new TGraphAsymmErrors(againstElectronMedium_n,againstElectronMedium_d);
 theAgainstElectronMediumRatio->BayesDivide(againstElectronMedium_n, againstElectronMedium_d);
 theAgainstElectronMediumRatio->SetTitle("Tau AgainstElectronMediumMVA5");
 theAgainstElectronMediumRatio->SetLineColor(lmcol+2);
 theAgainstElectronMediumRatio->SetMarkerColor(lmcol+2);
 string titleYaxis4;
  titleYaxis4 = "#epsilon(AgainstElectronMediumMVA5)";
 theAgainstElectronMediumRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theAgainstElectronMediumRatio->GetYaxis()->SetTitle(titleYaxis4.c_str());
 theAgainstElectronMediumRatio->GetYaxis()->SetRangeUser(0,1.25);

 TLegend *legend2 = new TLegend(0.65, 0.525, 0.9, 0.825);
 legend2->AddEntry(theAgainstElectronLooseRatio, "AgainstElectronLooseMVA5", "lep");
 legend2->AddEntry(theAgainstElectronMediumRatio, "AgainstElectronMediumMVA5", "lep");

 theAgainstElectronLooseRatio->Draw("PAZ9");
 theAgainstElectronMediumRatio->Draw("P same");
 legend2->Draw("same");
*/
/*
 TCanvas *c2 = new TCanvas();
 c2->cd();
 TGraphAsymmErrors* theAgainstElectronLooseRatio = new TGraphAsymmErrors(Tau_AgainstElectronLooseNMinus1_n,Tau_AgainstElectronLooseNMinus1_d);
 theAgainstElectronLooseRatio->BayesDivide(Tau_AgainstElectronLooseNMinus1_n, Tau_AgainstElectronLooseNMinus1_d);
 theAgainstElectronLooseRatio->SetTitle("Tau AgainstElectronMVA5 NMinus1");
 theAgainstElectronLooseRatio->SetLineColor(lmcol);
 theAgainstElectronLooseRatio->SetMarkerColor(lmcol);
 string titleYaxis3;
  titleYaxis3 = "#epsilon(AgainstElectronMVA5)";
 ...
 theAgainstElectronLooseRatio->GetYaxis()->SetTitle(titleYaxis3.c_str());
 theAgainstElectronLooseRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theAgainstElectronMediumRatio = new TGraphAsymmErrors(Tau_AgainstElectronMediumNMinus1_n,Tau_AgainstElectronMediumNMinus1_d);
 theAgainstElectronMediumRatio->BayesDivide(Tau_AgainstElectronMediumNMinus1_n, Tau_AgainstElectronMediumNMinus1_d);
 theAgainstElectronMediumRatio->SetTitle("Tau AgainstElectronMediumMVA5 NMinus1");
 theAgainstElectronMediumRatio->SetLineColor(lmcol+2);
 theAgainstElectronMediumRatio->SetMarkerColor(lmcol+2);
 string titleYaxis4;
  titleYaxis4 = "#epsilon(AgainstElectronMediumMVA5)";
...
/*
 TCanvas *c3 = new TCanvas();
 c3->cd();
 TGraphAsymmErrors* theAgainstMuonLoose3Ratio = new TGraphAsymmErrors(againstMuonLoose3_n,againstMuonLoose3_d);
 theAgainstMuonLoose3Ratio->BayesDivide(againstMuonLoose3_n, againstMuonLoose3_d);
 theAgainstMuonLoose3Ratio->SetTitle("Tau AgainstMuon3");
 theAgainstMuonLoose3Ratio->SetLineColor(lmcol);
 theAgainstMuonLoose3Ratio->SetMarkerColor(lmcol);
 string titleYaxis5;
  titleYaxis5 = "#epsilon(AgainstMuon3)";
 theAgainstMuonLoose3Ratio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theAgainstMuonLoose3Ratio->GetYaxis()->SetTitle(titleYaxis5.c_str());
 theAgainstMuonLoose3Ratio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theAgainstMuonTight3Ratio = new TGraphAsymmErrors(againstMuonTight3_n,againstMuonTight3_d);
 theAgainstMuonTight3Ratio->BayesDivide(againstMuonTight3_n, againstMuonTight3_d);
 theAgainstMuonTight3Ratio->SetTitle("Tau AgainstMuon3");
 theAgainstMuonTight3Ratio->SetLineColor(lmcol+2);
 theAgainstMuonTight3Ratio->SetMarkerColor(lmcol+2);
 string titleYaxis6;
  titleYaxis6 = "#epsilon(AgainstMuon3)";
 theAgainstMuonTight3Ratio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theAgainstMuonTight3Ratio->GetYaxis()->SetTitle(titleYaxis6.c_str());
 theAgainstMuonTight3Ratio->GetYaxis()->SetRangeUser(0,1.25);

 TLegend *legend3 = new TLegend(0.65, 0.525, 0.9, 0.825);
 legend3->AddEntry(theAgainstMuonLoose3Ratio, "AgainstMuonLoose3", "lep");
 legend3->AddEntry(theAgainstMuonTight3Ratio, "AgainstMuonTight3", "lep");

 theAgainstMuonLoose3Ratio->Draw("PAZ9");
 theAgainstMuonTight3Ratio->Draw("P same");
 legend3->Draw("same");
*/
...
/*
 TCanvas *c3 = new TCanvas();
 c3->cd();
 TGraphAsymmErrors* theAgainstMuonLoose3Ratio = new TGraphAsymmErrors(Tau_AgainstMuonLoose3NMinus1_n,Tau_AgainstMuonLoose3NMinus1_d);
 theAgainstMuonLoose3Ratio->BayesDivide(Tau_AgainstMuonLoose3NMinus1_n, Tau_AgainstMuonLoose3NMinus1_d);
 theAgainstMuonLoose3Ratio->SetTitle("Tau AgainstMuon3 NMinus1");
 theAgainstMuonLoose3Ratio->SetLineColor(lmcol);
 theAgainstMuonLoose3Ratio->SetMarkerColor(lmcol);
 string titleYaxis5;
  titleYaxis5 = "#epsilon(AgainstMuon3)";
 ...
 theAgainstMuonLoose3Ratio->GetYaxis()->SetTitle(titleYaxis5.c_str());
 theAgainstMuonLoose3Ratio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theAgainstMuonTight3Ratio = new TGraphAsymmErrors(Tau_AgainstMuonTight3NMinus1_n,Tau_AgainstMuonTight3NMinus1_d);
 theAgainstMuonTight3Ratio->BayesDivide(Tau_AgainstMuonTight3NMinus1_n, Tau_AgainstMuonTight3NMinus1_d);
 theAgainstMuonTight3Ratio->SetTitle("Tau AgainstMuon3 NMinus1");
 theAgainstMuonTight3Ratio->SetLineColor(lmcol+2);
 theAgainstMuonTight3Ratio->SetMarkerColor(lmcol+2);
 string titleYaxis6;
  titleYaxis6 = "#epsilon(AgainstMuon3)";
...
/*
 TCanvas *c4 = new TCanvas();
 c4->cd();
 TGraphAsymmErrors* theLooseCombinedIsoDB3HitsRatio = new TGraphAsymmErrors(looseCombinedIsoDB3Hits_n,looseCombinedIsoDB3Hits_d);
 theLooseCombinedIsoDB3HitsRatio->BayesDivide(looseCombinedIsoDB3Hits_n, looseCombinedIsoDB3Hits_d);
 theLooseCombinedIsoDB3HitsRatio->SetTitle("Tau CombinedIsoDB3Hits");
 theLooseCombinedIsoDB3HitsRatio->SetLineColor(lmcol);
 theLooseCombinedIsoDB3HitsRatio->SetMarkerColor(lmcol);
 string titleYaxis7;
  titleYaxis7 = "#epsilon(CombinedIsoDB3Hits)";
 theLooseCombinedIsoDB3HitsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theLooseCombinedIsoDB3HitsRatio->GetYaxis()->SetTitle(titleYaxis7.c_str());
 theLooseCombinedIsoDB3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theMediumCombinedIsoDB3HitsRatio = new TGraphAsymmErrors(mediumCombinedIsoDB3Hits_n,mediumCombinedIsoDB3Hits_d);
 theMediumCombinedIsoDB3HitsRatio->BayesDivide(mediumCombinedIsoDB3Hits_n, mediumCombinedIsoDB3Hits_d);
 theMediumCombinedIsoDB3HitsRatio->SetTitle("Tau CombinedIsoDB3Hits");
 theMediumCombinedIsoDB3HitsRatio->SetLineColor(lmcol+2);
 theMediumCombinedIsoDB3HitsRatio->SetMarkerColor(lmcol+2);
 string titleYaxis8;
  titleYaxis8 = "#epsilon(CombinedIsoDB3Hits)";
 theMediumCombinedIsoDB3HitsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theMediumCombinedIsoDB3HitsRatio->GetYaxis()->SetTitle(titleYaxis8.c_str());
 theMediumCombinedIsoDB3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theTightCombinedIsoDB3HitsRatio = new TGraphAsymmErrors(tightCombinedIsoDB3Hits_n,tightCombinedIsoDB3Hits_d);
 theTightCombinedIsoDB3HitsRatio->BayesDivide(tightCombinedIsoDB3Hits_n, tightCombinedIsoDB3Hits_d);
 theTightCombinedIsoDB3HitsRatio->SetTitle("Tau CombinedIsoDB3Hits");
 theTightCombinedIsoDB3HitsRatio->SetLineColor(lmcol+4);
 theTightCombinedIsoDB3HitsRatio->SetMarkerColor(lmcol+4);
 string titleYaxis9;
  titleYaxis9 = "#epsilon(CombinedIsoDB3Hits)";
 theTightCombinedIsoDB3HitsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theTightCombinedIsoDB3HitsRatio->GetYaxis()->SetTitle(titleYaxis9.c_str());
 theTightCombinedIsoDB3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);



 TLegend *legend4 = new TLegend(0.65, 0.525, 0.9, 0.825);
 legend4->AddEntry(theLooseCombinedIsoDB3HitsRatio, "LooseCombinedIsoDB3Hits", "lep");
 legend4->AddEntry(theMediumCombinedIsoDB3HitsRatio, "MediumCombinedIsoDB3Hits", "lep");
 legend4->AddEntry(theTightCombinedIsoDB3HitsRatio, "TightCombinedIsoDB3Hits", "lep");

 theLooseCombinedIsoDB3HitsRatio->Draw("PAZ9");
 theMediumCombinedIsoDB3HitsRatio->Draw("P same");
 theTightCombinedIsoDB3HitsRatio->Draw("P same");
 legend4->Draw("same");
*/
...
/*
 TCanvas *c4 = new TCanvas();
 c4->cd();
 TGraphAsymmErrors* theLooseCombinedIsoDB3HitsRatio = new TGraphAsymmErrors(Tau_LooseCombinedIsoDB3HitsNMinus1_n,Tau_LooseCombinedIsoDB3HitsNMinus1_d);
 theLooseCombinedIsoDB3HitsRatio->BayesDivide(Tau_LooseCombinedIsoDB3HitsNMinus1_n, Tau_LooseCombinedIsoDB3HitsNMinus1_d);
 theLooseCombinedIsoDB3HitsRatio->SetTitle("Tau CombinedIsoDB3Hits NMinus1");
 theLooseCombinedIsoDB3HitsRatio->SetLineColor(lmcol);
 theLooseCombinedIsoDB3HitsRatio->SetMarkerColor(lmcol);
 string titleYaxis7;
  titleYaxis7 = "#epsilon(CombinedIsoDB3Hits)";
 ...
 theLooseCombinedIsoDB3HitsRatio->GetYaxis()->SetTitle(titleYaxis7.c_str());
 theLooseCombinedIsoDB3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theMediumCombinedIsoDB3HitsRatio = new TGraphAsymmErrors(Tau_MediumCombinedIsoDB3HitsNMinus1_n,Tau_MediumCombinedIsoDB3HitsNMinus1_d);
 theMediumCombinedIsoDB3HitsRatio->BayesDivide(Tau_MediumCombinedIsoDB3HitsNMinus1_n, Tau_MediumCombinedIsoDB3HitsNMinus1_d);
 theMediumCombinedIsoDB3HitsRatio->SetTitle("Tau CombinedIsoDB3Hits NMinus1");
 theMediumCombinedIsoDB3HitsRatio->SetLineColor(lmcol+2);
 theMediumCombinedIsoDB3HitsRatio->SetMarkerColor(lmcol+2);
 string titleYaxis8;
  titleYaxis8 = "#epsilon(CombinedIsoDB3Hits)";
 ...
 theMediumCombinedIsoDB3HitsRatio->GetYaxis()->SetTitle(titleYaxis8.c_str());
 theMediumCombinedIsoDB3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theTightCombinedIsoDB3HitsRatio = new TGraphAsymmErrors(Tau_TightCombinedIsoDB3HitsNMinus1_n,Tau_TightCombinedIsoDB3HitsNMinus1_d);
 theTightCombinedIsoDB3HitsRatio->BayesDivide(Tau_TightCombinedIsoDB3HitsNMinus1_n, Tau_TightCombinedIsoDB3HitsNMinus1_d);
 theTightCombinedIsoDB3HitsRatio->SetTitle("Tau CombinedIsoDB3Hits NMinus1");
 theTightCombinedIsoDB3HitsRatio->SetLineColor(lmcol+4);
 theTightCombinedIsoDB3HitsRatio->SetMarkerColor(lmcol+4);
 string titleYaxis9;
  titleYaxis9 = "#epsilon(CombinedIsoDB3Hits)";
...
/*
 ...
 TCanvas *c5 = new TCanvas();
 c5->cd();
 TGraphAsymmErrors* theLoosePileupWeightedIso3HitsRatio = new TGraphAsymmErrors(loosePileupWeightedIso3Hits_n,loosePileupWeightedIso3Hits_d);
 theLoosePileupWeightedIso3HitsRatio->BayesDivide(loosePileupWeightedIso3Hits_n, loosePileupWeightedIso3Hits_d);
 theLoosePileupWeightedIso3HitsRatio->SetTitle("Tau PileupWeightedIso3Hits");
 theLoosePileupWeightedIso3HitsRatio->SetLineColor(lmcol);
 theLoosePileupWeightedIso3HitsRatio->SetMarkerColor(lmcol);
 string titleYaxis10;
  titleYaxis10 = "#epsilon(PileupWeightedIso3Hits)";
 theLoosePileupWeightedIso3HitsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theLoosePileupWeightedIso3HitsRatio->GetYaxis()->SetTitle(titleYaxis10.c_str());
 theLoosePileupWeightedIso3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theMediumPileupWeightedIso3HitsRatio = new TGraphAsymmErrors(mediumPileupWeightedIso3Hits_n,mediumPileupWeightedIso3Hits_d);
 theMediumPileupWeightedIso3HitsRatio->BayesDivide(mediumPileupWeightedIso3Hits_n, mediumPileupWeightedIso3Hits_d);
 theMediumPileupWeightedIso3HitsRatio->SetTitle("Tau PileupWeightedIso3Hits");
 theMediumPileupWeightedIso3HitsRatio->SetLineColor(lmcol+2);
 theMediumPileupWeightedIso3HitsRatio->SetMarkerColor(lmcol+2);
 string titleYaxis11;
  titleYaxis11 = "#epsilon(PileupWeightedIso3Hits)";
 theMediumPileupWeightedIso3HitsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theMediumPileupWeightedIso3HitsRatio->GetYaxis()->SetTitle(titleYaxis11.c_str());
 theMediumPileupWeightedIso3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);
 TGraphAsymmErrors* theTightPileupWeightedIso3HitsRatio = new TGraphAsymmErrors(tightPileupWeightedIso3Hits_n,tightPileupWeightedIso3Hits_d);
 theTightPileupWeightedIso3HitsRatio->BayesDivide(tightPileupWeightedIso3Hits_n, tightPileupWeightedIso3Hits_d);
 theTightPileupWeightedIso3HitsRatio->SetTitle("Tau PileupWeightedIso3Hits");
 theTightPileupWeightedIso3HitsRatio->SetLineColor(lmcol+4);
 theTightPileupWeightedIso3HitsRatio->SetMarkerColor(lmcol+4);
 string titleYaxis12;
  titleYaxis12 = "#epsilon(PileupWeightedIso3Hits)";
 theTightPileupWeightedIso3HitsRatio->GetXaxis()->SetTitle(titleXaxis.c_str());
 theTightPileupWeightedIso3HitsRatio->GetYaxis()->SetTitle(titleYaxis12.c_str());
 theTightPileupWeightedIso3HitsRatio->GetYaxis()->SetRangeUser(0,1.25);



 TLegend *legend5 = new TLegend(0.65, 0.525, 0.9, 0.825);
 legend5->AddEntry(theLoosePileupWeightedIso3HitsRatio, "LoosePileupWeightedIso3Hits", "lep");
 legend5->AddEntry(theMediumPileupWeightedIso3HitsRatio, "MediumPileupWeightedIso3Hits", "lep");
 legend5->AddEntry(theTightPileupWeightedIso3HitsRatio, "TightPileupWeightedIso3Hits", "lep");

 theLoosePileupWeightedIso3HitsRatio->Draw("PAZ9");
 theMediumPileupWeightedIso3HitsRatio->Draw("P same");
 theTightPileupWeightedIso3HitsRatio->Draw("P same");
 legend5->Draw("same");
